...
class AlbumsScreen extends ConsumerWidget {
  ...
  Future<List<_AlbumGroup>> _loadAlbums(WidgetRef ref, List<File> files) async {
    ...
    for (final file in files) {
      try {
        ...
        final artist =
            (tag?.trackArtist ?? '').trim().isNotEmpty
                ? tag!.trackArtist!.trim()
                : ((tag?.albumArtist ?? '').trim().isNotEmpty
        ...
        final key = album.isEmpty ? 'Unknown Album' : album;
        grouped
        ...
        final fallbackTitle = dot > 0 ? fileName.substring(0, dot) : fileName;
        grouped
            .putIfAbsent('Unknown Album', () => <_TrackMeta>[])
            .add(
              _TrackMeta(
                file: file,
                title: fallbackTitle,
    ...
    for (final entry in sorted) {
      ...
      result.add(
        _AlbumGroup(
          name: entry.key,
          tracks: tracks,
          previewText: representative.artist,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<_AlbumGroup>>(
        future: _loadAlbums(ref, files),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }

          final albums = snapshot.data ?? const <_AlbumGroup>[];
          if (albums.isEmpty) {
            return const Center(
              child: Text('No albums found. Import audio files first.'),
            );
          }
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final subtitle =
                  '${album.tracks.length} track(s) | ${album.previewText}';
              ...
              return Padding(
                ...
                child: Material(
                  ...
                  child: InkWell(
                    ...
                    onTap: () => _showTracksSheet(context, ref, album),
                    child: Ink(
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(28),
                        border: Border.all(
